...
export default function ReturnSeatSelect() {
  ...
  // Guard: Check if selectedReturnLeg is valid
  useEffect(() => {
    if (!selectedReturnLeg || !Array.isArray(selectedReturnLeg) || selectedReturnLeg.length === 0) {
      // Show "No return operator selected" state is handled in the render phase
    }
  }, [selectedReturnLeg, navigate])

  // Initialize seat selections for all return legs with default first seat type
  useEffect(() => {
    if (selectedReturnLeg && Array.isArray(selectedReturnLeg)) {
      const initial = {}
      selectedReturnLeg.forEach((op, i) => {
        initial[i] = {
          seats: [],
  ...
  if (!selectedReturnLeg || !Array.isArray(selectedReturnLeg) || selectedReturnLeg.length === 0) {
    return (
      <div className="bg-surface text-on-surface min-h-screen flex flex-col">
        ...
        `}</style>
        <Navbar />
        <main className="pt-28 pb-12 px-4 md:px-12 max-w-7xl mx-auto flex-grow flex items-center justify-center">
          <div className="text-center py-20 text-on-surface-variant font-medium">
            ...
            <button onClick={() => navigate('/search/return')}
              className="ml-2 text-primary font-bold underline bg-transparent border-none cursor-pointer">
  ...
  const activeLeg = selectedReturnLeg[activeLegIndex]
  ...
  const canProceed = selectedReturnLeg.length > 0 && selectedReturnLeg.every((_, i) => {
  ...
  const handleProceed = () => {
    ...
    const enrichedLegs = selectedReturnLeg.map((op, i) => ({
      ...op,
    ...
    selectReturnLeg(enrichedLegs)
    const return_transport = enrichedLegs.reduce((sum, leg) => sum + (leg.totalFare || 0), 0)
        ...
        <header className="mb-12">
          <div>
            ...
            <p className="text-on-surface-variant mt-2 text-lg">
              {searchParams.returnDate || searchParams.date || ''} • Return Leg Journey
            </p>
          </div>
        </header>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-on-surface-variant font-medium">
                          Leg {i + 1}: {op.leg?.from || op.from} → {op.leg?.to || op.to}
                        </p>
                      </div>
                    </div>
            ...
            {activeLeg && (
              <div className="bg-surface-container-lowest rounded-2xl editorial-shadow p-8 sticky top-28">
                <h2 className="text-2xl font-headline font-bold mb-2 text-on-surface">
                  Return Seats — Leg {activeLegIndex + 1}: {activeLeg.leg?.from || activeLeg.from} → {activeLeg.leg?.to || activeLeg.to}
                </h2>
                <p className="text-xs text-on-surface-variant font-semibold uppercase tracking-wider mb-6">
                  {activeLeg.operator_name}
                </p>
